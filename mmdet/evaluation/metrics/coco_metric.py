...
from ..functional import eval_bbox_cgf1, eval_recalls
...
class CocoMetric(BaseMetric):
    ...
    Args:
        ...
        metric (str | List[str]): Metrics to be evaluated. Valid metrics
            include 'bbox', 'segm', 'proposal', 'proposal_fast', and
            'cgf1'. cgF1 is evaluated on bounding boxes.
            Defaults to 'bbox'.
        ...
        cgf1_score_threshold (float): Minimum detection score included in
            bbox cgF1 evaluation. Defaults to 0.5.
        cgf1_use_cats (bool): Whether bbox cgF1 query matching is
            category-aware. Defaults to True.
    """
    ...
    def __init__(self,
                 ...
                 cgf1_score_threshold: float = 0.5,
                 cgf1_use_cats: bool = True) -> None:
        super().__init__(collect_device=collect_device, prefix=prefix)
        # coco evaluation metrics
        self.metrics = metric if isinstance(metric, list) else [metric]
        allowed_metrics = [
            'bbox', 'segm', 'proposal', 'proposal_fast', 'cgf1'
        ]
        for metric in self.metrics:
            if metric not in allowed_metrics:
                raise KeyError(
                    "metric should be one of 'bbox', 'segm', 'proposal', "
                    f"'proposal_fast', 'cgf1', but got {metric}.")

        # do class wise evaluation, default False
        self.classwise = classwise
        # whether to use multi processing evaluation, default False
        self.use_mp_eval = use_mp_eval
        self.cgf1_score_threshold = float(cgf1_score_threshold)
        self.cgf1_use_cats = bool(cgf1_use_cats)
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            if metric == 'cgf1':
                ...
                cgf1_results = eval_bbox_cgf1(
                    gt_data=self._coco_api.dataset,
                    pred_data=predictions,
                    image_ids=self.img_ids,
                    cat_ids=self.cat_ids,
                    score_threshold=self.cgf1_score_threshold,
                    use_cats=self.cgf1_use_cats)
                eval_results.update(cgf1_results)
                mode = ('category-aware'
                        if self.cgf1_use_cats else 'category-agnostic')
                logger.info(
                    f'bbox cgF1 metric ({mode}, score threshold='
                    f'{self.cgf1_score_threshold:.3f}):\n'
                    f' cgF1 @[ IoU=0.50:0.95] = '
                    f'{cgf1_results["cgF1_bbox"]:.3f}\n'
                    f' cgF1 @[ IoU=0.50     ] = '
                    f'{cgf1_results["cgF1_bbox50"]:.3f}\n'
                    f' cgF1 @[ IoU=0.75     ] = '
                    f'{cgf1_results["cgF1_bbox75"]:.3f}\n'
                    f' positive_micro_F1          = '
                    f'{cgf1_results["bbox_positive_micro_F1"]:.3f}\n'
                    f' IL_MCC                     = '
                    f'{cgf1_results["bbox_IL_MCC"]:.3f}')
                continue
