...
# The original config is 150k iterations for large mixed datasets.
# This is a 15-class finetune config. Validate every eval_interval and select
# checkpoints by downstream APmask/cgF1, not by the final iteration.
max_iter = 100000
eval_interval = 10000
train_cfg = dict(
    _delete_=True,
...
custom_hooks = [
    dict(
        type='ODVGValLossAndMetricHook',
        interval=eval_interval,
        split='valid',
        output='train_eval_records.jsonl',
        batch_size=2,
        num_workers=2,
        amp=True)
]
